...
struct AmountGoalProgressBar: View {
    var habit: HabitDefinition
    
    let currentValue: Float
    let isLoading: Bool
        
    let onAddAmount: (Float) -> Void
    ...
    var body: some View {
        VStack{
            HStack{
                ...
                Text(habit.name)
                Spacer()
                Text("\(currentValue.trimmedString) / \(targetValue.trimmedString) \(habit.targetValueUnit!.abbreviation)")
                    .font(.caption)
                if progress < 1.0 {
